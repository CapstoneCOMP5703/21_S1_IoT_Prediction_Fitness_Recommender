...
downloaded = drive.CreateFile({'id': '1-0inpPfUV1q7PSmgBN5mz5cueTJMu0_a'})
downloaded.GetContentFile('processed_endomondoHR_proper_interpolate_2w.csv')
...
class dataInterpreter(object):
    def __init__(self, T, inputAtts, includeUser, includeSport, includeTemporal, targetAtts,
                 ...
                 trainValidTestSplit=[.8, .1, .1], zMultiple=5, trainValidTestFN=None):
        self.T = T
        self.filename = fn
        self.data_path = "."
    ...
    def preprocess_data(self):
        ...
        self.output_dim = len(
            self.targetAtts)  # each continuous target has dimension 1, so total length = total dimension
    ...
    def dataIteratorSupervised(self, trainValidTest):
        ...
        for idx in indices:
            ...
            num_steps = len(
                current_input['distance'].replace('[', '').replace(']', '').replace('Decimal(\'', '').replace('\')',
                                                                                                              '').split(
                    ','))
            ...
            for att_idx, att in enumerate(inputAtts):
                if att == 'time_elapsed':
                    ...
                    inputs[att_idx, :] = current_input[att].replace('[', '').replace(']', '').replace('Decimal(\'',
                                                                                                      '').replace('\')',
                                                                                                                  '').split(
                        ',')[:num_steps]
            for att in targetAtts:
                outputs[0, :] = current_input[att].replace('[', '').replace(']', '').replace('Decimal(\'', '').replace(
                    '\')', '').split(',')[:num_steps]
            ...
            if self.includeTemporal:
                ...
                for att_idx, att in enumerate(inputAtts):
                    if att == 'time_elapsed':
                        ...
                        context_inputs[att_idx, :] = context_input[att].replace('[', '').replace(']', '').replace(
                            'Decimal(\'', '').replace('\')', '').split(',')[:num_steps]
                for att in targetAtts:
                    context_outputs[0, :] = context_input[att].replace('[', '').replace(']', '').replace('Decimal(\'',
                                                                                                         '').replace(
                        '\')', '').split(',')[:num_steps]
            ...
            for idx in self.trainingSet:
                if (type(self.original_data.loc[idx]['distance']) == str):
                    data_len = data_len + (len(
                        self.original_data.loc[idx]['distance'].replace('[', '').replace(']', '').replace('Decimal(\'',
                                                                                                          '').replace(
                            '\')', '').split(',')) - self.T)
                else:
                    data_len = data_len + 1
    ...
    def loadTrainValidTest(self):
        with open(self.trainValidTestFN, "rb") as f:
            ...
            print("train/valid/test set size = {}/{}/{}".format(len(self.trainingSet), len(self.validationSet),
                                                                len(self.testSet)))
            ...
            for index, currData in self.original_data.iterrows():
                ...
                for att in self.isSequence:
                    ...
                    if (type(currData[att]) == str):
                        for num in currData[att].replace('[', '').replace(']', '').replace('Decimal(\'', '').replace(
                                '\')', '').split(','):
    ...
    def computeMeanStd(self, varSums, numDataPoints, attributes):
        ...
        for numDataPoints, currData in self.original_data.iterrows():
            ...
            for att in attributes:
                ...
                if (type(currData[att]) == str):
                    dataArray = []
                    for item in currData[att].replace('[', '').replace(']', '').replace('Decimal(\'', '').replace('\')',
                                                                                                                  '').split(
                            ','):
    ...
    def forward(self, attr_inputs, context_embedding, input_variable):
        ...
        for t in range(self.T):
            ...
            attn_weights = F.softmax(x.view(-1, self.input_size),
                                     dim=-1)  # batch_size * input_size, attn weights with values sum up to 1.
            # Eqn. 10: LSTM
...
class da_rnn:
    def __init__(self, encoder_hidden_size=64, decoder_hidden_size=64, T=10,
        ...
        print("include pretrain/user/sport/temporal = {}/{}/{}/{}".format(self.pretrain, self.includeUser,
                                                                          self.includeSport, self.includeTemporal))
        ...
        self.trainValidTestFN = self.data_path.split(".")[0] + "_temporal_dataset_updated_test_9:1.pkl"
        ...
        if self.includeTemporal:
            ...
            self.context_encoder = contextEncoder(input_size=self.input_dim + 1, hidden_size=encoder_hidden_size,
                                                  output_size=self.context_dim).cuda()
        ...
        self.encoder_optimizer = optim.Adam([
            {'params': [param for name, param in self.encoder.named_parameters() if 'user_embedding' in name],
             'weight_decay': wd1},
            {'params': [param for name, param in self.encoder.named_parameters() if 'sport_embedding' in name],
             'weight_decay': wd2},
        ...
        self.context_encoder_optimizer = optim.Adam(
            params=filter(lambda p: p.requires_grad, self.context_encoder.parameters()),
            lr=learning_rate)
        ...
        for iteration in range(n_epochs):
            ...
            for batch, training_batch in enumerate(trainDataGen):
                ...
                attr_inputs, context_input_1, context_input_2, input_variable, y_history, y_target = self.get_batch(
                    training_batch)
                loss = self.train_iteration(attr_inputs, context_input_1, context_input_2, input_variable, y_history,
                                            y_target)
            ...
            print(val_batch_num)
            print('-' * 89)
            print('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.3f}'.format(iteration,
...
torch.save(model, '/content/gdrive/My Drive/model_epoch_02.pt')
