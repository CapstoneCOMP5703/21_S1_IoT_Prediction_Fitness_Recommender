...
class Model:
    ...
    def predict_data(self, userId, calories):
        ...
        if len(tmp_df) == 0:  # identify new users
            run_data = np.asarray([2, calories, 155, 11]).reshape(1, -1)
            run_res = self.run_clf.predict(run_data)[0]
            bike_data = np.asarray([2, calories, 145, 26]).reshape(1, -1)
            bike_res = self.run_clf.predict(bike_data)[0]
            mountain_data = np.asarray([2, calories, 150, 20]).reshape(1, -1)
            mountain_res = self.run_clf.predict(mountain_data)[0]
        else:
            try:
...
if __name__ == '__main__':
    ...
    RandomForestRegressor(n_estimators=100, max_depth=4, min_samples_split=2)
    # print
    print(data)
